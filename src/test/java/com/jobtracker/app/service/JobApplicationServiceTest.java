...
class JobApplicationServiceTest {
  ...
  void shouldAddApplication() {
    ...
    JobApplication result =
        service.addApplication("Google", "Software Engineer", "Applied via referral");
  ...
  void shouldReturnAllApplications() {
    ...
    when(repository.findAllByOrderByAppliedDateDesc()).thenReturn(applications);

    List<JobApplication> result = service.getAllApplications();

    assertEquals(1, result.size());

    verify(repository).findAllByOrderByAppliedDateDesc();
  }
  ...
  void shouldFindApplicationById() {
    ...
    when(repository.findById(1)).thenReturn(Optional.of(application));
  ...
  void shouldThrowExceptionWhenApplicationNotFound() {
    ...
    assertThrows(ApplicationNotFoundException.class, () -> service.findById(999));
  }
  ...
  void shouldUpdateStatus() {
    ...
    service.updateStatus(1, ApplicationStatus.INTERVIEW);
  ...
  void shouldReturnStatistics() {
    List<JobApplication> applications =
        List.of(
            new JobApplication(
                1, "Google", "SDE", ApplicationStatus.APPLIED, LocalDate.now(), null),
            new JobApplication(
                2, "Amazon", "SDE", ApplicationStatus.INTERVIEW, LocalDate.now(), null));

    when(repository.findAllByOrderByAppliedDateDesc()).thenReturn(applications);

    var statistics = service.getStatistics();

    assertEquals(1L, statistics.get(ApplicationStatus.APPLIED));

    assertEquals(1L, statistics.get(ApplicationStatus.INTERVIEW));
  }
  ...
  void shouldThrowExceptionWhenDeletingUnknownApplication() {
    ...
    assertThrows(ApplicationNotFoundException.class, () -> service.deleteApplication(999));
  }
  ...
  void shouldThrowExceptionWhenUpdatingUnknownApplication() {
    ...
    assertThrows(
        ...
        () -> service.updateStatus(999, ApplicationStatus.INTERVIEW));
  }
  ...
  void shouldSearchApplicationsByCompany() {
    ...
    when(repository.findByCompanyContainingIgnoreCase("goo")).thenReturn(applications);

    List<JobApplication> result = service.searchByCompany("goo");

    assertEquals(1, result.size());
    assertEquals("Google", result.getFirst().getCompany());

    verify(repository).findByCompanyContainingIgnoreCase("goo");
  }
}
